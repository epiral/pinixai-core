...
class TodoWebClip extends Clip {
  name = "todo-web";
  domain = "productivity";
  patterns = [
    "list -> add -> list (从 Web 或 API 添加后验证)",
    "list -> delete -> list (从 Web 或 API 删除后验证)",
  ];
  ...
  add = handler(
    z.object({
      title: z.string().trim().min(1).describe("Todo 标题"),
    }),
    z.object({
      todo: todoSchema,
    }),
